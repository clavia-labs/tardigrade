...
import type { NativeTool } from "@flamecast/harness"
import { invoiceForOrder } from "./invoices"

// A native tool is a spec plus a handler. Both live with the native-tools module, so a code
// candidate can change the surface and implementation together.
//
// A handler that fails does not fail the turn: the native-tools module records the failure as
// `ToolReturned` with an `error`, and the model reads it and tries something else.

export const lookupInvoice: NativeTool = {
  spec: {
    name: "lookup_invoice",

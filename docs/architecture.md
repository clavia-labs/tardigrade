...
A result shaped like an answer is checked before it is read as one. An answer stopped at its token ceiling is a fragment, recorded as `AnswerTruncated`, and the turn continues from it rather than starting the work again. What the framework records is what happened: the fragment, its cost, and the tool call that was cut. What to say to the model about it is a module's decision, because it depends on what the agent was building. A tool call whose recorded arguments will not parse would reach the tool with none of what the model asked for.

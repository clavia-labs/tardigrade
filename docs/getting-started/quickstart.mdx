...
Code mode defaults to `DEFAULT_CODE_TOOL_CONCURRENCY`, which is `1`. Set `codeMode(components, { toolConcurrency })` to control admission of `execute` calls. Code execution follows the code scheduler. Custom model integrations supply Effect's `LanguageModel` service. The agent binding translates its stream into response evidence and tool requests. The interpreter records those requests before tool execution.
</Tip>

...
<Filesystem root="tardie-agent/">
  ...
  - `models.lock.json` Resolved runtime model definitions
...
Before the first Cloudflare deployment, set the secrets from the [Cloudflare setup guide](/docs/platforms/cloudflare#configure). Then deploy the agent:

...
```ts
import type { ComponentOutput, TransitionContext } from "tardie/core"
import type { Event } from "tardie/core/event"

interface ComponentDefinition<State, View, Requirements = never, Result = unknown> {
  readonly name: string
  readonly initial: () => State
  readonly step: (state: Readonly<State>, event: Event, context: TransitionContext) => State
  readonly output: (state: Readonly<State>) => ComponentOutput<View, Requirements, Result>
}
```

...
Agentic software is a whole different beast compared to traditional software. It is highly nondeterministic, and its actions can have consequences far beyond the system itself. Traditional ways of testing often fall short in providing enough guarantees for an agent's behaviors. As engineers building these systems, how then do we get back our sleep?
...
export const testServices = ({ generate }: ActorCheckContext) => {
  ...
  return Layer.merge(factoryLayer(), model)
}
```
...
Once we've seen the counterexample to our safety rule, we can have conditions within our actor's tools to prevent such paths. Replace `factoryLayer` in `factory-test-model.ts` with the guarded version below. The production tool uses this service to reject a batch if the tank wasn't inspected or the batch would cause a wastewater spill.
...
const factoryLayer = () => {
  ...
  let inspected = false
  const capacity = 10
  const wastePerBatch = 6

  return Layer.succeed(Factory, {
    inspectWaterTank: Effect.sync((): FactoryResult => {
      inspected = true
    ...
    producePaperClip: Effect.sync((): FactoryResult => {
      ...
      if (storedWaste + wastePerBatch > capacity) {
        return { action: "denied", reason: "Treat wastewater before production." }
      }
      storedWaste += wastePerBatch
      inspected = false
      return { action: "produced", wasteAdded: wastePerBatch }
    }),
    treatWasteWater: Effect.sync((): FactoryResult => {
      const wasteRemoved = storedWaste
      storedWaste = 0
...
Denied actions leave the tank unchanged. After treatment, Clippy must inspect again before producing another batch.

...
The Haskell community addressed this with the [IO monad](https://www.haskell.org/tutorial/io.html), which represents I/O actions as values to compose and execute later. [Effect](https://effect.website/docs/v4/getting-started/the-effect-type) follows the same idea.
...
## Supplying interactions to children

A parent can supply typed input constructors when building its children. The infer builder exposes `message`, which describes an agent turn. The alarm package chooses its response through `onFired`:
...
Builder capabilities pass through wrappers such as budget and tools. A captured capability cannot be used outside the component subtree supplying it; binding that request throws. Callbacks and event constructors must be pure because projection and replay may evaluate them repeatedly. Returning `undefined` from `onFired` proposes no response.

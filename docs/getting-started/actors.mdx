...
For catalog selection and project configuration, use `bunModelServices({ env, model: { providerLayer, configure } })` and return its `layers` from `layersFor`. Worker applications pass `workerModelServices({ model: { providerLayer, configure } })` to `defineWorkerHost`. Both helpers accept `ModelIntegrationOptions` from `tardie/model/host` under `model`. Its `configure(selected)` callback supplies request limits and native provider settings. Project configuration parsing checks that `models.providers.<provider>.models.<model>.options` is a JSON object. The selected provider validates those options and the host overrides with its native configuration schema during assembly, before HTTP. Unselected providers are not loaded for validation.

When mounting `compaction()`, supply model capacity through the host model services. They resolve `contextWindowTokens` from the selected model's catalog metadata, with corrections under `models.providers.<provider>.models.<model>.contextWindowTokens`. A custom model layer supplies the same metadata through `ModelSelection.resolve` from `tardie/model/settings`. Missing capacity fails before inference. `compaction()` controls rendering limits and the trigger and retention ratios. Its optional `model` selects the summarizer, with the host default used when omitted. This choice does not change the conversation budget. `triggerRatio` and `retainRatio` default to `0.8` and `0.5`. `fireRatio` and `keepRatio` are compatibility aliases; conflicting declarations are rejected.

...
const meeseeks = await host.allocateChildThread({
...
```json
{ "actor": "tardie", "instance": "rick", "thread": "researcher" }
```
...
Once a thread is allocated, a caller can use its reference to invoke a method defined by the actor. For example, `rickMain` identifies Rick's `"main"` thread, where a caller can invoke `tardie`'s `message` method.
...
Calls through the host use ordinary `await` calls. The `key` identifies the invocation so a retry can reuse the same call and its result. Calls from inside an actor action use the runtime-aware API and `yield*` to participate in that action's replay history.
...
const server = await serve(host, { port: 3000 })
```

Call Rick's `main` thread from another process:

```sh
curl -X POST 'http://localhost:3000/v1/actors/rick/threads/main/methods/message' \
...
<div className="actor-event-table">
  <table>
    ...
    <tbody>
      ...
      <tr><td><code>ThreadRequested</code></td><td><code>meeseeks</code></td><td><code>researcher</code></td><td>2</td></tr>
    </tbody>
  </table>
</div>

`main` and `lab` are root threads at depth 0. `researcher` is a child of `main`, and `meeseeks` is a child of `researcher`. The host builds the instance's thread directory from these records.

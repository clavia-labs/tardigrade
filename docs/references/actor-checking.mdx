...
Install `fast-check` in the project that runs the checks with `bun add -d fast-check`, then import from `tardie/testing`. The runtime package declares `fast-check` as an optional peer dependency. [Full example](/docs/verifying-actors).

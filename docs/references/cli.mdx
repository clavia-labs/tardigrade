...
| Port for `dev` | `--port` | `PORT` | `4242`, then a lower available port |
...
Initialization and setup use [models.dev](https://models.dev) for the searchable provider and model catalog. The first interactive run saves the validated public catalog in `.tardigrade/models.json`. Later runs can use that snapshot. `tdg dev` refreshes the same cache when its server starts and keeps the resolved snapshot in memory.

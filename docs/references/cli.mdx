...
<div className="cli-command-list">
  ...
  <div className="cli-command-row"><code>tdg fork &lt;thread&gt;</code><p>Copy a thread prefix through a checkpoint onto a new root</p></div>
...
Setup preserves unrelated platform settings, JSONC comments, local secret entries, and other provider connections. When a provider uses a new protocol, import its adapter in `server.ts` and `worker.ts` and add it to `modelAdapters(...)`. Host startup reports an unregistered protocol.
...
`tdg fork <thread> --until <id|seq>` copies that thread's prefix through a checkpoint onto a new root. `--until` is a 1-based sequence or an event id. `--name` assigns the destination; omit it for a host-generated identity. See [Fork a thread for an experiment](../how-to/fork-experiments.md).

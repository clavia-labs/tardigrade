...
The generated `bun run dev` script loads `.dev.vars` and starts `server.ts`. It stores actor threads in `.tardigrade/actor.sqlite` under the actor directory. Keep that directory present until the server stops. A deployment reads credentials from its platform secret store. Edit `server.ts` to supply application Effect services or change the server composition. `tdg dev` remains available for projects that use the bundled API and Voyager server.

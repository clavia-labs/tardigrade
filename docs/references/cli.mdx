...
For scripted setup, include `models` in the provider configuration:

```sh
tdg setup provider localhost '{"protocol":"openai-chat-completions","baseUrl":"http://localhost:8080/v1","env":["API_KEY"],"models":{"qwen-local":{"metadata":{"contextWindowTokens":32768,"toolCall":true}}}}'
tdg setup default --provider localhost --model qwen-local
```

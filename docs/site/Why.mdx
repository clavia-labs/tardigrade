...
If you can describe the behavior you want to see, you should be able to define it as a function of the event log. An agent harness could then be viewed as a composition of such behaviors.
...
Composition is possible because each state machine is defined as a typed state machine using [Effect TS](https://effect.website/).
...
interface ComponentDefinition<State, View, Requirements = never> {
  ...
  // initial creates the component's starting state.
  readonly initial: () => State
  // step derives the next state from an event.
  readonly step: (state: State, event: Event) => State
  // output derives the component's view and enabled transitions.
  readonly output: (state: State) => {
    // view is the value observed by the parent.
    readonly view: View
    // transitions are the work enabled by the component.
    readonly transitions: ReadonlyArray<Transition<never, Requirements>>
  }
  // cancelState derives transitions when an invocation is cancelled.
  readonly cancelState?: (
    state: State,
    cancellation: InvocationCancellation
  ) => ReadonlyArray<Transition<never, Requirements>>
  // keys derives stable keys for the component's events.

...
For deployment, install either the [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/install-and-update/) for Cloudflare or the [Celld CLI](https://celld.dev/docs/) for your own cloud.
...
After your call, you will see `.tardigrade/actor.sqlite`. This SQLite database holds the actor's event logs. An event log is an immutable, append-only ledger of events sent to the actor and events generated by the actor.
...
These events are stored locally and persist across runs. Each thread has its own event log within the same SQLite database.

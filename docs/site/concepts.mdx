...
<ConceptSection kind="components">
  ...
  <div className="compaction-example-copy">
    ...
    <p>At 104k tokens, the log has crossed the 80% threshold of a 128k window. Compaction derives a transition that summarizes the older span. The transition appends `CompactionCompleted` to the log, and later model requests use its summary with a 64k-token tail. The full log remains unchanged.</p>
  </div>

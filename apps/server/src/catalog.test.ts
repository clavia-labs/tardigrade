...
import { loadModelRegistry, modelCatalogOf, type ModelRegistryLoadOptions } from "@clavia/tardigrade-model/registry"
...
const options = (fetcher: typeof fetch, policy: ModelRegistryLoadOptions["policy"] = "refresh"): ModelRegistryLoadOptions => ({
...
const run = (
  effect: ReturnType<typeof loadModelRegistry>,
...
describe("loadModelRegistry", () => {
  test("cache-first fetches once and reuses the validated snapshot", async () => {
    ...
    const loaded = await run(loadModelRegistry(options(answering(source, { etag: "catalog-7" }), "cache-first")), repository)
    ...
    const cached = await run(loadModelRegistry(options(refused, "cache-first")), repository)
  ...
  test("cache-first reads only the actor model scope", async () => {
    ...
    const refused = (async () => { throw new Error("source should not be called") }) as unknown as typeof fetch
    const loaded = await run(loadModelRegistry({
  ...
  test("a failed refresh serves the last valid snapshot", async () => {
    const repository = layerMemoryModelCatalogRepository()
    await run(loadModelRegistry(options(answering(source, { etag: "catalog-7" }))), repository)
    const failed = (async () => { throw new Error("source unavailable") }) as unknown as typeof fetch
    const loaded = await run(loadModelRegistry(options(failed)), repository)
  ...
  test("an invalid refresh cannot replace the last valid snapshot", async () => {
    const repository = layerMemoryModelCatalogRepository()
    await run(loadModelRegistry(options(answering(source, { etag: "catalog-7" }))), repository)
    const loaded = await run(loadModelRegistry(options(answering({}, { etag: "catalog-8" }))), repository)
    expect(loaded.snapshot).toMatchObject({ revision: "catalog-7", status: "cached" })
    const cached = await run(loadModelRegistry(options(answering(source), "cache-first")), repository)
  ...
  test("reports unavailable when neither source nor cache is valid", async () => {
    const failed = (async () => { throw new Error("source unavailable") }) as unknown as typeof fetch
    const loaded = await run(loadModelRegistry(options(failed)), layerMemoryModelCatalogRepository())

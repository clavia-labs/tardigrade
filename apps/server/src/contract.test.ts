...
const layerThreadsEmpty = Layer.succeed(Threads)({
  ...
  ensure: () => Effect.succeed({ methods: {}, sqlite: ":memory:", append: () => Effect.void, events: () => Effect.succeed([]), eventsPage: () => Effect.succeed([]), awaitHead: () => Effect.never, actorEventsPage: () => Effect.succeed([]), actorHead: Effect.succeed(0), awaitActorHead: () => Effect.never, list: Effect.succeed([]), settled: Effect.void }),

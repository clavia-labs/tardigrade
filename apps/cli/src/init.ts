...
export const initActor = async (name: string, options: InitActorOptions): Promise<InitializedActor> => {
  ...
  try {
    ...
    await writeFile(modelLock, `${JSON.stringify(options.modelLock ?? await emptyModelLock(), null, 2)}\n`, "utf8")
  } catch (error) {
    await rm(directory, { recursive: true, force: true })
    throw error
  }

  return { name, directory, entry, server, worker, manifest, celldManifest, packageManifest, modelLock, template: options.template ?? DEFAULT_INIT_TEMPLATE }
}

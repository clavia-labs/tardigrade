...
import type { CodePolicy } from "./policy"
...
const settled = async (head: Event): Promise<ReadonlyArray<Event>> => {
  ...
  return Effect.runPromise(
    Effect.gen(function* () {
      yield* settleActor({ projections: [executionFor({}, [worldPackage])], keyOf: composeKeys(messageKeys, codeKeys) })
...
describe("package call failure policy", () => {
  ...
  test("a transiently failing call retries before the body sees an answer", async () => {
    ...
    const events = await Effect.runPromise(
      Effect.gen(function* () {
        yield* settleActor({
          projections: [executionFor({ call: { retryDelaysMs: [0] } }, [flakyPackage])],
  ...
  test("a hanging call exhausts its deadline and backoff as one durable error", async () => {
    ...
    const events = await Effect.runPromise(
      Effect.gen(function* () {
        yield* settleActor({
          projections: [executionFor({ call: { attemptTimeoutMs: 10, retryDelaysMs: [1, 2] } }, [hangingPackage])],
          keyOf: composeKeys(messageKeys, codeKeys)
        })
        return yield* Effect.flatMap(EventLog, (l) => l.read)
      }).pipe(Effect.provide(Layer.mergeAll(memoryLog(log), jsSandbox, KeyValueStore.layerMemory))) as Effect.Effect<ReadonlyArray<Event>>
...
describe("the spill bound", () => {
  ...
  test("a stated bound spills a value the default would have kept whole", async () => {
    ...
    const drive = (reactor: typeof emptyExecution) =>
      Effect.runPromise(
        Effect.gen(function* () {
    ...
    const spilled = (await drive(executionFor({ spill: { spillBytes: 10, previewChars: 4 } }, []))).find(
    ...
    const whole = (await drive(emptyExecution)).find((e) => e.type === "CodeSettled") as { tmp?: string; result?: unknown }
...
describe("the pointer's note", () => {
  ...
  const drive = (log: Event[], reactor: typeof emptyExecution) =>
    Effect.runPromise(
      Effect.gen(function* () {
        yield* settleActor({ projections: [reactor], keyOf: composeKeys(messageKeys, codeKeys) })
        return yield* Effect.flatMap(EventLog, (l) => l.read)
  ...
  test("a scope with no workspace package gets a note with no verb", async () => {
    const events = await drive(bigLog(), executionFor({ spill: { spillBytes: 10, previewChars: 4 } }, []))
  ...
  test("a stated note overrides the derived one", async () => {
    const events = await drive(
      bigLog(),
      executionFor({ spill: { spillBytes: 10, previewChars: 4, note: (ref) => `files.open({ref: '${ref}'})` } }, [])
    )
    const settle = events.find((e) => e.type === "CodeSettled") as { note?: string }
  ...
  test("a workspace whose read takes no ref refuses at construction", () => {
    ...
    expect(() => executionFor({}, [shadow])).toThrow(/spill pointer/)
    // A stated note lifts the refusal: the consumer then owns the words.
    expect(() => executionFor({ spill: { note: (ref) => ref } }, [shadow])).not.toThrow()
  })
  ...
  test("a workspace without the advertised grep refuses at construction", () => {
    ...
    expect(() => executionFor({}, [shadow])).toThrow(/workspace\.grep/)
  })
})
...
describe("a package's requirements ride its type", () => {
  ...
  test("a package method reads its service through the funnel", async () => {
    ...
    const events = await Effect.runPromise(
      Effect.gen(function* () {
        yield* settleActor({ projections: [executionFor({}, [tickerPackage])], keyOf: composeKeys(messageKeys, codeKeys) })
        return yield* Effect.flatMap(EventLog, (l) => l.read)
      }).pipe(
        Effect.provide(
          Layer.mergeAll(
  ...
  test("the reactor a service-needing package builds cannot stand where the service is missing", () => {
    ...
    const powered: TransitionProjection<unknown, KeyValueStore.KeyValueStore | Ticker> = executionFor({}, [tickerPackage])
    expectTypeOf(powered).not.toMatchTypeOf<TransitionProjection<unknown, KeyValueStore.KeyValueStore>>()
  ...
  test("two packages under one name fail at construction", () => {
    ...
    // methods the code reaches by list order (execute.ts, executionFor).
    expect(() => executionFor({}, [tickerPackage, { ...tickerPackage, description: "another" }])).toThrow(
...
test("execution refs isolate package replay and spill storage when payload IDs repeat", async () => {
  ...
  const events = await Effect.runPromise(Effect.gen(function* () {
    const definition = { projections: [executionFor({ spill: { spillBytes: 10, previewChars: 4 } }, [pkg])], keyOf: composeKeys(messageKeys, codeKeys) }
...
const executionFor = <const P extends ReadonlyArray<import("../package/definition").CodeComponent<unknown>>>(policy: Partial<CodePolicy>, children: P) => {
  const child = codeExecution(children, policy)
  machineOf(child).initial()

...
test("an external reply resumes a parked package after cold replay", async () => {
  const invoked: string[] = []
  const nativePackage = definePackage({
    name: "native",
    description: "checks a durable native receipt",
    methods: {
      read: (_args, ctx) => Effect.gen(function* () {
        invoked.push(ctx.callId)
        const events = yield* Effect.flatMap(EventLog, (log) => log.read)
        if (!events.some((event) => event.type === "ExternalReplyReceived" && event.id === "native:ready")) {
          return yield* new Park({ callId: ctx.callId, awaiting: "native:ready" })
        }
        return { value: "retained" }
      })
    }
  })
  const definition = { projections: [codeReactorFor({}, [nativePackage])], keyOf: composeKeys(messageKeys, codeKeys) }
  const drive = (history: ReadonlyArray<Event>) => Effect.runPromise(
    Effect.gen(function* () {
  ...
  const parked = await drive([
    ...
    { type: "CodeDispatched", execId: "e1", code: "return await native.read({})", turn: "t1", at: 2 }
  ])
  expect(parked.filter((event) => event.type === "BlockedOn")).toHaveLength(1)
  expect(parked.some((event) => event.type === "CodeSettled")).toBe(false)

  const cold = JSON.parse(JSON.stringify([
    ...parked,
    externalReplyReceived({ id: "native:other", at: 3 })
  ])) as Event[]
  const stillParked = await drive(cold)
  expect(stillParked.some((event) => event.type === "PackageReturned")).toBe(false)

  const resumed = await drive(JSON.parse(JSON.stringify([
    ...stillParked,
    externalReplyReceived({ id: "native:ready", at: 4 })
  ])) as Event[])
  expect(resumed.filter((event) => event.type === "PackageCalled")).toHaveLength(1)
  expect(resumed.filter((event) => event.type === "BlockedOn")).toHaveLength(1)
  expect(resumed.filter((event) => event.type === "PackageReturned")).toHaveLength(1)
  expect(resumed.filter((event) => event.type === "CodeSettled")).toHaveLength(1)
  expect(new Set(invoked)).toEqual(new Set(["e1.0"]))
  expect(invoked).toHaveLength(2)

  const settledAgain = await drive(resumed)
  expect(settledAgain.filter((event) => event.type === "PackageCalled")).toHaveLength(1)
  expect(settledAgain.filter((event) => event.type === "PackageReturned")).toHaveLength(1)
  expect(invoked).toHaveLength(2)
})

test("an external reply recorded before parking remains ready", async () => {
  let calls = 0
  const nativePackage = definePackage({
    name: "native",
    description: "parks once after readiness was recorded",
    methods: {
      read: (_args, ctx) => ++calls === 1
        ? Effect.fail(new Park({ callId: ctx.callId, awaiting: "native:early" }))
        : Effect.succeed({ value: "retained" })
    }
  })
  const events = await Effect.runPromise(
    Effect.gen(function* () {
      yield* settleActor({ projections: [codeReactorFor({}, [nativePackage])], keyOf: composeKeys(messageKeys, codeKeys) })
      return yield* Effect.flatMap(EventLog, (log) => log.read)
    }).pipe(Effect.provide(Layer.mergeAll(memoryLog([
      { type: "MessageReceived", id: "t1", text: "go", at: 1 },
      externalReplyReceived({ id: "native:early", at: 2 }),
      { type: "CodeDispatched", execId: "e1", code: "return await native.read({})", turn: "t1", at: 3 }
    ]), jsSandbox, KeyValueStore.layerMemory))) as Effect.Effect<ReadonlyArray<Event>>
  )
  expect(calls).toBe(2)
  expect(events.filter((event) => event.type === "PackageCalled")).toHaveLength(1)
  expect(events.filter((event) => event.type === "BlockedOn")).toHaveLength(1)
  expect(events.filter((event) => event.type === "PackageReturned")).toHaveLength(1)
  expect(events.filter((event) => event.type === "CodeSettled")).toHaveLength(1)
})

test("two identical external calls resume by their distinct package identities", async () => {
  ...
  const recordPackage = definePackage({
    ...
    methods: {
      submit: (args, ctx) => Effect.gen(function* () {
        const events = yield* Effect.flatMap(EventLog, (log) => log.read)
        const ready = events.some((event) => event.type === "ExternalReplyReceived" && event.id === `record:${ctx.callId}`)
        physical.push({ callId: ctx.callId, state: ready ? "completed" : "pending" })
        if (!ready) return yield* new Park({ callId: ctx.callId, awaiting: `record:${ctx.callId}` })
  ...
  const drive = (history: ReadonlyArray<Event>) => Effect.runPromise(
    Effect.gen(function* () {
      yield* settleActor({
        projections: [codeReactorFor({}, [recordPackage])],
        keyOf: composeKeys(messageKeys, codeKeys)
      })
      return yield* Effect.flatMap(EventLog, (log) => log.read)
    }).pipe(Effect.provide(Layer.mergeAll(memoryLog(JSON.parse(JSON.stringify(history)) as Event[]), jsSandbox, KeyValueStore.layerMemory))) as Effect.Effect<ReadonlyArray<Event>>
  )
  let events = await drive([
    { type: "MessageReceived", id: "turn", text: "submit twice", at: 1 },
    {
      type: "CodeDispatched",
      execId: "action",
      code: "await record.submit({value: 'same'}); return await record.submit({value: 'same'})",
      turn: "turn",
      at: 2
    }
  ])
  expect(physical).toEqual([{ callId: "action.0", state: "pending" }])

  events = await drive(events)
  events = await drive(events)
  expect(physical).toEqual([{ callId: "action.0", state: "pending" }])

  events = await drive([...events, externalReplyReceived({ id: "record:action.0", at: 3 })])
  expect(physical).toEqual([
    { callId: "action.0", state: "pending" },
    { callId: "action.0", state: "completed" },
    { callId: "action.1", state: "pending" }
  ])
  ...
  events = await drive([...events, externalReplyReceived({ id: "record:action.1", at: 4 })])
  expect(physical).toEqual([
    { callId: "action.0", state: "pending" },
    { callId: "action.0", state: "completed" },
    { callId: "action.1", state: "pending" },
    { callId: "action.1", state: "completed" }
  ])
  const called = events.filter((event) => event.type === "PackageCalled")
  expect(called.map((event) => event.callId)).toEqual(["action.0", "action.1"])
  expect(called.map((event) => event.arguments)).toEqual([{ value: "same" }, { value: "same" }])
  expect(events.filter((event) => event.type === "PackageReturned").map((event) => event.callId)).toEqual([
    "action.0",
    "action.1"
  ])
  expect(events.filter((event) => event.type === "CodeSettled")).toHaveLength(1)

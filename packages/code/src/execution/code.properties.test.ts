...
const runAttempt = async (completions: ReadonlyArray<Completion>): Promise<ReadonlyArray<Event> | undefined> => {
  ...
  const running = Effect.runPromise(
    Effect.gen(function* () {
      yield* settleActor({
        projections: [transitionProjectionOf(codeExecution([probePackage]))],
...
test("a parked attempt cannot record late guest requests", async () => {
  await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 8 }), async count => {
    let stopped: import("../sandbox/service").SandboxCall | undefined
    const log: Event[] = [
      { type: "MessageReceived", id: "turn", text: "wait", at: 0 },
      { type: "CodeDispatched", execId: "execution", code: "", turn: "turn", at: 1 }
    ]
    const environment = Layer.mergeAll(
      KeyValueStore.layerMemory,
      Layer.succeed(EventLog, withWatermark({ read: Effect.sync(() => [...log]), append: events => Effect.sync(() => { log.push(...events) }) })),
      Layer.succeed(Sandbox, { run: (_code, bindings) => Effect.promise(async () => {
        const calls = bindings.probe as Readonly<Record<string, import("../sandbox/service").SandboxCall>>
        stopped = calls.finish!
        await stopped({ kind: "park", delay: 0 }, 0)
        return new Promise<never>(() => undefined)
      }) })
    )
    await Effect.runPromise(settleActor({ projections: [transitionProjectionOf(codeExecution([probePackage]))], keyOf: composeKeys(messageKeys, codeKeys) }).pipe(Effect.provide(environment)))
    const before = [...log]
    expect(stopped).toBeDefined()
    for (let ordinal = 1; ordinal <= count; ordinal++) {
      expect(await stopped!({ kind: "ready", delay: 0 }, ordinal)).toEqual({ _tag: "Parked" })
    }

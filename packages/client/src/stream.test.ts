...
import { actorStream, actorStreamUrl, CLOSED, stream, streamUrl, type EventSourceLike, type Frame } from "./stream"
import type { ActorEventRow, EventRow } from "./contract"
...
describe("streamUrl", () => {
  ...
  test("the actor stream carries its actor and cursor", () => {
    expect(actorStreamUrl("http://localhost:4111/", "one two", 4))
      .toBe("http://localhost:4111/v1/actors/one%20two/events/stream?after=4")
  })
})
...
describe("actorStream", () => {
  test("a threads frame becomes an actor event row", () => {
    const rows: Array<ActorEventRow> = []
    let source: FakeSource | undefined
    const unsubscribe = actorStream({
      baseUrl: "http://localhost:4111",
      actor: "main",
      after: 2,
      onEvent: (row) => rows.push(row),

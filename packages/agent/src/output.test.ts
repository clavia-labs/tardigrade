...
describe("the supported profile", () => {
  ...
  test("a schema deeper than the type-level reading is still checked at run time", () => {
    // Sixteen levels of nesting, four past the compile-time bound. The rule holds at the bottom.
...
describe("the declaration on the log", () => {
  ...
  // is a verdict the reactor turns into a terminal (runtime/infer.ts).

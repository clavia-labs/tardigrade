...
test("Bedrock feeds unknown calls back while executing valid siblings once", async () => {
  ...
  const host = createHost({ actorName: "recovery", actorFor: () => definition, layersFor: () => Layer.mergeAll(testModelLockLayer, KeyValueStore.layerMemory, layer.pipe(Layer.provide(FetchHttpClient.layer))) })

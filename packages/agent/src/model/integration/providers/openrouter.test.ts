...
for (const outcome of ["calls", "length", "malformed"] as const) test(`OpenRouter persists ${outcome} evidence and replays native reasoning`, async () => {
  ...
  const action = await Effect.runPromise(Effect.gen(function* () {
    ...
    return yield* runModelActor(infer, { model: reference, identity: { actor: "test", instance: "main", thread: "root", turn: "m1" }, system: "Read", trajectory: [], tools: [{ name: "read", description: "Read", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } }] })

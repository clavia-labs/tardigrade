...
for (const terminal of ["eof", "done", "complete", "recovered"] as const) {
  for (const tool of [false, true]) {
    test(`compat requires provider completion (terminal: ${terminal}, tool: ${tool})`, async () => {
      ...
      const layer = inferenceLayer({ provider: "openai-compat", endpoint: "https://fixture.invalid", client: { apiUrl: "https://fixture.invalid" }, model: { model: "fixture" }, retry: { backoffMs: [0], retryAfterJitterMs: 0 } }).pipe(Layer.provide(FetchHttpClient.layer))
      const result = await Effect.runPromise(Effect.flatMap(inferenceClient, (infer) => runModelActor(infer, { identity: { actor: "test", instance: "main", thread: "root", turn: "t" }, system: "Read", trajectory: [], tools: [{ name: "read", description: "Read", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } }] })).pipe(Effect.provide(layer), Effect.provideService(FetchHttpClient.Fetch, fetch)))
      if (terminal === "complete" || terminal === "recovered") {
        ...
        expect(result).toMatchObject({ kind: "fail", failure: { attempts: 1 } })
...
for (const provider of ["openai", "anthropic"] as const) {
  test(`${provider}: early EOF retries before exposing calls`, async () => {
    ...
    const layer = inferenceLayer({ provider, endpoint: "https://fixture.invalid", client: { apiUrl: "https://fixture.invalid" }, model: { model: "fixture" }, retry: { backoffMs: [0], retryAfterJitterMs: 0 } }).pipe(Layer.provide(FetchHttpClient.layer))
    const result = await Effect.runPromise(Effect.flatMap(inferenceClient, (infer) => runModelActor(infer, { identity: { actor: "test", instance: "main", thread: "root", turn: "t" }, system: "Read", trajectory: [], tools: [{ name: "read", description: "Read", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } }] })).pipe(Effect.provide(layer), Effect.provideService(FetchHttpClient.Fetch, fetch)))
    expect(requests).toBe(2)

...
for (const outcome of ["success", "refused", "truncated", "trailing-data", "unreported-retry", "exhausted"] as const) {
  test(`provider accounting survives ${outcome}`, async () => {
    ...
    const action = await Effect.runPromise(Effect.gen(function* () {
      ...
      return yield* runModelActor(infer, { identity: { actor: "test", instance: "main", thread: "root", turn: "m1" }, system: "Read", trajectory: [], tools: [{ name: "read", description: "Read", inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } }] })

...
describe("modelRequest tool and prompt policy", () => {
  ...
  // binding decides whether the model ever reads it (packages/agent/src/model/execution/output.ts, outputSystemFor).
  ...
  test("once the budget is spent, execute is dropped and the nudge is added", () => {
    ...
    expect(req.system).toContain("Your tool budget is spent.")
  })
  ...
  // (output.ts, DeclaredOutput; packages/agent/src/model/execution/output.ts, outputPreflight).
...
describe("the repair exchange in the render", () => {
  ...
  // (output.ts, OutputFallback; component/infer/machine.ts, openRejection).
...
describe("the tool surface decides the tool table", () => {
  ...
  test("every policy still folds over a swapped surface", () => {
    ...
    expect(spentReq.system).toContain("Your tool budget is spent.")
  })

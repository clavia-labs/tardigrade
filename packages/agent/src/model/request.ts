...
import { renderMessages, renderConversation, type AgentMessage } from "../projection/messages"
import {
  declaredOutputOf,
  type OutputContract,
  type OutputFallback
...
// the provider's own response-format surface (packages/agent/src/model/execution/output.ts, outputSchemaFor).
//
...
export type OutputRequest =
  | {
      ...
      // (packages/agent/src/model/execution/output.ts, outputModeOf).

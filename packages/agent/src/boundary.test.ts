...
describe("outputOf", () => {
  ...
  test("a running turn and a failed turn have no result to read", () => {
  ...
  test("a recorded result that misses its contract is loud, never a value nobody can trust", () => {
    const log: ReadonlyArray<Event> = [
      { type: "MessageReceived", id: "m1", text: "go", output: declaration, at: 0 },
  ...
  // Holding a contract is not evidence that a result was produced under it. Without this check a

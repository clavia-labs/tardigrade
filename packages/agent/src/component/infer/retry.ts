...
// RetrySchedule records the next attempt's wake time, retry index, and optional model switch (integration/infer-retry.test.ts, integration/infer-fallback.test.ts).
...
// retryDelayOf honors provider waits within the configured limit (retry.properties.test.ts).
...
// canFallback admits provider failures while leaving local contract failures terminal (retry.test.ts).

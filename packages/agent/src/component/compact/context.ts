...
export interface ContextPolicy {
  ...
  // fileTokens is the per-attachment estimate used to choose compaction cuts (compact.test.ts).
...
export interface CompactionPolicy {
  ...
  readonly keepRatio?: number
  readonly summaryLineCap: number
  // model selects the summarizer; omission uses the host default independently of conversation capacity (component/infer/infer.test.ts).
...
// ratioOf normalizes deprecated aliases and rejects conflicting declarations (compact.properties.test.ts).

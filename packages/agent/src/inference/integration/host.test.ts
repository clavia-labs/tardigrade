...
for (const provider of ["openai", "anthropic"] as const) {
  for (const explicit of [false, true]) test(`${provider}: host selection, native settings, and observer delivery (explicit: ${explicit})`, async () => {
    ...
    const catalog = { snapshot: { source: "models.dev" as const, revision: "r1", refreshedAt: 1, status: "fresh" as const, providers: [
      { id: reference.provider, name: "Private", env: [], models: [{ id: reference.model_id, metadata: { contextWindowTokens: 200000, maxOutputTokens: 2048, pricing: { promptUsdPerToken: 1, completionUsdPerToken: 2, cachedPromptUsdPerToken: 1 } } }] }
    ] } }
    ...
    const fetch = Object.assign(async (input: Parameters<typeof globalThis.fetch>[0], init?: RequestInit) => {
      ...
      expect(body).toMatchObject(provider === "openai" ? { model: reference.model_id, max_output_tokens: 2048, reasoning: { effort: "high" } } : { model: reference.model_id, max_tokens: 2048, thinking: { type: "enabled", budget_tokens: 1024 } })
    ...
    const binding = modelLayer(config, catalog, {
      ...
      configure: () => ({ maxOutputTokens: 8192, retry: { backoffMs: [] }, ...(provider === "openai" ? { openai: { max_output_tokens: 4096, reasoning: { effort: "high" } } } : { anthropic: { max_tokens: 4096, thinking: { type: "enabled", budget_tokens: 1024 } } }) }),

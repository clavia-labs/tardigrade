...
import { messageSubjects } from "../interaction/provider-message"
...
export const compileActor = <R>(
  ...
  const subjectFragments = components.flatMap((component) => component.subjects === undefined ? [] : [component.subjects])
  ...
  const subjectsOf = composeSubjects(messageSubjects, ...subjectFragments)

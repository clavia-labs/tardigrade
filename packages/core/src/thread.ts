...
import { ActorAddress, type ActorAddress as ActorAddressType } from "./communication/address"
...
export interface ThreadLineage {
  readonly parent: ActorAddressType
...
export const ThreadCreated = Schema.Struct({
  ...
  parent: Schema.optional(ActorAddress),
  depth: ThreadDepth,
  at: Schema.Finite
})
...
export const isThreadCreated = (event: Event | undefined): event is ThreadCreated => {
  ...
  return Schema.is(ActorAddress)(value.address) &&
    (value.parent === undefined || Schema.is(ActorAddress)(value.parent)) &&
...
// sameActorAddress compares actor identity without serializing it.
export const sameActorAddress = (left: ActorAddressType, right: ActorAddressType): boolean =>
...
export const sameThreadLineage = (created: ThreadCreated, lineage: ThreadLineage): boolean =>
  created.parent !== undefined && sameActorAddress(created.parent, lineage.parent) && created.depth === lineage.depth

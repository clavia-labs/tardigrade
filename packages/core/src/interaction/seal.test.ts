...
import { METHOD_SEALED_EVENT_TYPE, methodIsSealed, methodSealKeys, methodSealKey, methodSealOf, methodSealed } from "./seal"
import { actorInvocationContextFrom, methodIngressKeyOf, type ActorInvocation } from "./invocation"
import {
...
describe("method seals", () => {
  ...
  test("a seal owns its key fragment, so the host keys and refuses on it", () => {
    const seal = methodSealed({ method: "message", at: 1 })
    expect(methodSealKeys.keyOf(seal)).toBe(methodSealKey("message"))
    expect(methodSealKey("message")).toBe(`mseal:${JSON.stringify("message")}`)
    expect(methodSealKeys.keyOf({ type: "MessageReceived", id: "m1", at: 1 } as Event)).toBeUndefined()
    expect(methodIngressKeyOf(seal)).toBeUndefined()
...
// durable context, so direct enumeration sees it (invocation.ts, actorInvocationContextFrom).
...
describe("seal drains", () => {
  test("a seal enumerates direct and linked invocations and reads each disposition", () => {
    ...
    const log = [
      ...
      { type: "InvocationLinked", parent, child: { invocation: child }, target: "ag:main:child", at: 8 } as Event,
    ...
    expect(cancellationDispositionOf(log, work, parent)).toBe("requestable")
    expect(hasUnsettledInvocationChildren(log, parent)).toBe(true)

    // Once the child call settles on its target thread, the family closes and the drain owes nothing.
    const settled = [...log, { type: "ResponseReceived", method: child.method, call: child.id, from: "ag:main:child", at: 10 } as Event]
    expect(unsettledInvocationParentsOf(settled)).toEqual([])
    expect(hasUnsettledInvocationChildren(settled, parent)).toBe(false)
    expect(cancellationDispositionOf(settled, work, parent)).toBe("settled")
  })
})
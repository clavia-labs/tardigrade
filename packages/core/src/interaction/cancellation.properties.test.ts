...
import { actorCancellationProjection, cancellationMethodFor, cancellationKeys, cancellationTransitionsOf, cancelsInvocation } from "./cancellation"
import { alarmFired, initialMethodTimeoutState, methodTimeoutTransitions, reduceMethodTimeoutState } from "./timeout"
...
const sameInvocation = (left: InvocationRef, right: InvocationRef): boolean =>
  left.method === right.method && left.id === right.id && left.epoch === right.epoch

const started = (invocation: InvocationRef, at: number): Event => ({
  type: "InvocationStarted",
  invocation,
  at
}) as Event

const cancelled = (invocation: InvocationRef, at: number): Event => ({
  type: "InvocationCancelled",
  invocation,
  at
}) as Event

const invocationOf = (event: Event): InvocationRef | undefined =>
  (event as { readonly invocation?: InvocationRef }).invocation
...
const projectedMethod = (options: {
  ...
  const projection = {
    ...
    output: (state: { readonly started: ReadonlySet<string>; readonly cancelled: ReadonlySet<string> }) => ({
      currentEpoch: () => 0,
      invocationState: (invocation: InvocationRef) => {
        const key = JSON.stringify(invocation)
        if (!state.started.has(key)) return undefined
        return state.cancelled.has(key)
          ? { status: "cancelled" as const, cause: "requested" as const }
      ...
      ...(options.cancellationState
        ? {
          cancellationState: (invocation: InvocationRef) => {
            const key = JSON.stringify(invocation)
            if (!state.started.has(key)) return undefined
  ...
  const definition = {
    input: Schema.Void,
    output: Schema.Void,
    event: ({ invocation, at }: { readonly invocation: InvocationRef; readonly at: number }) => started(invocation, at),
    projection
  }
...
const request = (id: string, invocation: InvocationRef, at: number): Event => ({
...
describe("cancellation properties", () => {
  ...
  test("the control projection agrees with complete cancellation replay", () => {
    ...
    const childRequest = 'cancel:["x1","worker:main:child","inspect","m1",0]'
  ...
  test("ChildrenCancelled follows only InvocationLinked edges from the exact parent", () => {
    ...
    for (const mismatch of [
      ...
      { from: "worker:main:child", epoch: 1 }
    ]) {
      const events = [...initial, {
        type: "ResponseReceived", id: "wrong.reply", method: "$cancel", call: childRequest,

...
import { composeComponents, component as defineComponent, legacyComponent, type Component, type ViewAlgebra } from "../index"
...
describe("components", () => {
  test("composition combines views and concatenates transitions in component order", () => {
    ...
    expect(replayProjection(machineOf(composed), log).view).toEqual({ names: ["left", "right"] })
    expect(replayProjection(machineOf(composed), log).transitions.map((owed) => owed.key)).toEqual(["left", "right"])
  })
  ...
  test("composition reconciles the complete transition set", () => {
    ...
    expect(replayProjection(machineOf(composed), log).transitions.map((owed) => owed.key)).toEqual(["right"])
  ...
  test("reconciliation sees intents and external effects in one transition set", () => {
    ...
    expect(replayProjection(machineOf(composed), []).transitions.map((transition) => transition.key)).toEqual(["decision"])
  ...
  test("composition refuses work a reconciler did not receive", () => {
    ...
    expect(() => replayProjection(machineOf(composed), [{ type: "Ready" }])).toThrow(
  ...
  test("the empty composition derives the algebra's empty view and no work", () => {
    ...
    expect(replayProjection(machineOf(composed), [])).toEqual({ view: facts.empty, transitions: [] })
  })
  ...
  test("composition is associative for view and transition order", () => {
    ...
    const right = component("right", "Ready")
    const log: ReadonlyArray<Event> = [{ type: "Ready" }]
    const leftGrouped = replayProjection(machineOf(composeComponents("left-grouped", facts, [
      composeComponents("left-middle", facts, [left, middle]),
      right
    ])), log)
    const rightGrouped = replayProjection(machineOf(composeComponents("right-grouped", facts, [
      left,
  ...
  test("composition preserves cancellation projections in component order", () => {
    const cancellable = (name: string): Component<Facts> => legacyComponent({
      ...
      name,
      derive: () => ({ view: facts.empty, transitions: [], interactions: {
    ...
    expect((machineOf(composed).output(replayState(machineOf(composed), [])).interactions?.cancel?.({
      request: "x1",
  ...
  test("the component exposes its transition projection", () => {
    ...
    expect(replayProjection(transitionProjectionOf(source), log).map((owed) => owed.key)).toEqual(
      replayProjection(machineOf(source), log).transitions.map((owed) => owed.key)
    )
  })
  ...
  test("composition preserves incremental child projections", () => {
    ...
    expect(machineOf(composed)).toBeDefined()
    ...
    expect(replayProjection(machineOf(composed), [{ type: "Ready" }]).view).toEqual({ names: ["left", "right"] })
  })
  ...
  test("composition reuses branches whose child state identities are stable", () => {
    ...
    const projection = machineOf(composed)
  ...
  test("cached composition passes child state to cancellation projections", () => {
    const child = defineComponent({
      ...
      output: (state) => ({ view: facts.empty, transitions: [], interactions: {
        cancel: () => state.ctx === undefined ? [] : [state.ctx.intent("cancel", { type: "Cancelled", count: state.count })]
    } }),
    ...
    expect(projection.output(state).interactions?.cancel?.({
      request: "x1",

...
`e2e/actor/component-input.test.ts` runs a dummy parent and child through the memory and SQLite hosts. It verifies a complete input round trip through a wrapper, then seeds each recorded protocol prefix into a fresh host with freshly constructed components. Recovery must produce the same request identities and result, and repeated wakes must leave the log unchanged. This exercises replay recovery; it does not simulate process termination or filesystem failure.

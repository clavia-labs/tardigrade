...
| `runtime/CancellationDeadline` | A cancellation deadline bounds parent settlement when a child does not acknowledge | `CancellationDeadline.cfg` | None |
| `runtime/CancellationParallel` | Independent cleanup obligations can start before their peers finish | `CancellationParallel.cfg` | `CancellationSerial.cfg` |

...
test("host storage deduplicates runtime completions within each owning log", async () => {
  ...
  for (const thread of ["a", "b"]) {
    ...
    const requestIndex = events.findIndex((event) => event.type === "Requested")
    expect(requestIndex).toBeGreaterThanOrEqual(0)
    const completion = events.find((event) => event.type === "Executed")!
    expect(completion).toMatchObject({ type: "Executed", transitionRef: { seq: requestIndex + 1, component: "worker", tag: "execute" }, result: "done" })

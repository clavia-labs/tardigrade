...
for (const entry of ["../host.ts", "../stream/collect.ts", "../catalog/index.ts", "./anthropic.ts", "./openai.ts", "./openai-compat.ts", "./openrouter.ts"]) test(`${entry} does not resolve agent, client, AWS, or Smithy`, async () => {
  ...
  const bundle = await Bun.build({
    ...
    target: "browser",
    plugins: [{ name: "enforce-model-boundary", setup(build) {
      build.onResolve({ filter: /^(?:@clavia\/tardigrade-(?:agent|client)(?:\/|$)|@tardie\/ai-bedrock(?:\/|$)|(?:@aws-sdk|@smithy)\/)/ }, ({ path }) => {
        resolved.push(path)
        throw new Error(`Disallowed model dependency: ${path}`)
      })
    } }]
  })
...
test("an explicitly supplied provider rejects a mismatched protocol before dispatch", () => {
  ...
  expect(() => openrouter({ ...options, provider: "openai-compat" })).toThrow("supply the matching providerLayer")

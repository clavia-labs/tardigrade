...
Use the generated quickstart as the starting point for a new actor. In an interactive terminal, initialization asks for the provider connection, credential, and default model:
...
In a non-interactive terminal, pass `--provider`, `--base-url`, `--driver`, `--credential-env`, and `--default-model`. The named credential must already be present in the environment. Initialization writes a matching `actor.ts`, `tardigrade.jsonc`, and `.env`.
...
Keep the server running. Discover the actor's methods and call one from another shell in the same directory:
...
Open the trace URL printed by `tdg call` to inspect the live trajectory in Voyager.
...
```bash
tdg call message '{"text":"Investigate the failure"}' --actor researcher --url http://localhost:4241
```

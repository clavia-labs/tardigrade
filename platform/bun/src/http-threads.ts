...
export const bunHttpThreads = (host: BunHost, options: {
  ...
  readKey: (thread, key) => Effect.promise(() => host.readKey(thread, key)),
  readSubject: (thread, subject) => Effect.promise(() => host.readSubject(thread, subject)),
...
export const bunHttpServices = (host: object) => {
  ...
  return Context.make(Threads, {
    ...
    readKey: (instance, thread, key) => Effect.flatMap(ensure(instance), (threads) => threads.readKey(thread, key)),
    readSubject: (instance, thread, subject) => Effect.flatMap(ensure(instance), (threads) => threads.readSubject(thread, subject)),

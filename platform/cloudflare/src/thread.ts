...
export class ThreadDO extends DurableObject<Env> {
  ...
  private async openHost(): Promise<CloudflareThreadHost> {
    ...
    const commitObserver = mountedActor?.commitObserverFor?.({ env: this.env, storage: this.ctx.storage, actorInstance, thread: currentThread })
    return createCloudflareThreadHost({
      ...
      layers: (() => {
        ...
        const observer = mountedActor?.inferenceObserverFor?.({ env: this.env, storage: this.ctx.storage, actorInstance, thread })
        const framework = Layer.mergeAll(modelLayer(models, modelScope, observer), FetchHttpClient.layer, sandboxLayer)
        const application = mountedActor?.layersFor?.({ env: this.env, storage: this.ctx.storage, actorInstance, thread })
        return application === undefined ? framework : Layer.mergeAll(framework, application)
      })(),
      routes: [independentRoute],
      ...(mountedActor?.storeFor === undefined ? {} : { store: mountedActor.storeFor({ env: this.env, storage: this.ctx.storage, actorInstance, thread: currentThread }) }),

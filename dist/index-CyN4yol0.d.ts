...
interface NativeTool<R = never> {
  readonly spec: NativeToolSpec;
  readonly run: (input: unknown, context?: NativeToolContext) => Effect.Effect<unknown, never, R>;
}
...
//#endregion
//#region packages/harness/src/dependency.d.ts
declare const TokenValue: unique symbol;
type Projection<Value> = (log: ReadonlyArray<Envelope>) => Value;
interface Token<Id extends string, Value> {
  readonly id: Id;
  readonly [TokenValue]?: Value;
}
type AnyToken = Token<string, unknown>;
type ValueOf<T extends AnyToken> = T extends Token<string, infer Value> ? Value : never;
interface Binding<T extends AnyToken> {
  readonly token: T;
  readonly project: Projection<ValueOf<T>>;
}
declare const token: <const Id extends string, Value>(id: Id) => Token<Id, Value>;
declare const provide: <T extends AnyToken>(token: T, project: Projection<ValueOf<T>>) => Binding<T>;
interface ModuleContext<Requires extends readonly AnyToken[]> {
  readonly resolve: <T extends Requires[number]>(token: T, log: ReadonlyArray<Envelope>) => ValueOf<T>;
}
//#endregion
...
interface Nudge {
  ...
  readonly nativeTools?: ReadonlyArray<NativeToolSpec> | ((log: ReadonlyArray<Envelope>) => ReadonlyArray<NativeToolSpec>);
...
declare const resolve: <T extends AnyToken>(program: Pick<AgentProgram<never>, "bindings">, token: T, log: ReadonlyArray<Envelope>) => ValueOf<T>;
...
interface Module<Id extends string = string, Provides extends readonly Binding<AnyToken>[] = readonly [], Requires extends readonly AnyToken[] = readonly [], R = never> {
  readonly id: Id;
  readonly version?: string;
  readonly fingerprint?: unknown;
...
declare const defineModule: <const Id extends string, const Provides extends readonly Binding<AnyToken>[] = readonly [], const Requires extends readonly AnyToken[] = readonly [], R = never>(module: Module<Id, Provides, Requires, R>) => Module<Id, Provides, Requires, R>;
type ModuleId<One> = One extends Module<infer Id, any, any, any> ? Id : never;
type ProvidedToken<One> = One extends Module<string, infer Provides, any, any> ? Provides[number]["token"] : never;
type RequiredToken<One> = One extends Module<string, any, infer Requires, any> ? Requires[number] : never;
type TokenId<One> = One extends Token<infer Id, unknown> ? Id : never;
type MissingDependencies<Modules extends readonly unknown[]> = Exclude<TokenId<RequiredToken<Modules[number]>>, TokenId<ProvidedToken<Modules[number]>>>;
...
type ValidModules<Modules extends readonly unknown[]> = [MissingDependencies<Modules>] extends [never] ? [DuplicateModuleIds<Modules>] extends [never] ? unknown : {
...
declare const morphCompaction: (options?: MorphOptions) => Module<"compaction", readonly [], readonly [Token<"inference.state", InferenceState>], never>;
//#endregion
...
declare const inferenceState: Token<"inference.state", InferenceState>;
...
declare const inference: (options?: InferenceOptions) => Module<"inference", readonly [Binding<Token<"inference.state", InferenceState>>], readonly [], never>;
//#endregion
...
declare const defaultPack: <R = never>(options?: DefaultPackOptions<R>) => readonly [Module<"inference", readonly [Binding<Token<"inference.state", InferenceState>>], readonly [], never>, Module<"native-tools", readonly [], readonly [], R>, Module<"budget", readonly [], readonly [], never>, Module<"contract", readonly [], readonly [], never>, Module<"compaction", readonly [], readonly [Token<"inference.state", InferenceState>], never>];
//#endregion
//#region packages/harness/src/render.d.ts
declare const nativeToolSurface: (render: RenderPlan, log: ReadonlyArray<Envelope>) => ReadonlyArray<NativeToolSpec>;
...
//#endregion
//#region packages/harness/src/modules/native-tools.d.ts
declare const nativeTools: <R = never>(list: ReadonlyArray<NativeTool<R>>) => Module<"native-tools", readonly [], readonly [], R>;
interface AgentNativeToolOptions {
  readonly name: string;
  readonly description: string;
...
declare const agentNativeTool: (options: AgentNativeToolOptions) => NativeTool<Router>;
//#endregion
...
//#endregion
export { usedOf as $, renderMessages as A, token as At, KEEP_RATIO as B, NativeToolCall as Bt, transcript as C, AnyToken as Ct, usageIn as D, Token as Dt, turnView as E, Projection as Et, inference as F, InferenceProvider as Ft, BudgetOptions as G, inferWith as Gt, TRIGGER_RATIO as H, NativeToolSpec as Ht, inferenceState as I, InferenceSelection as It, budgetOf as J, BudgetPhase as K, selectedInference as Kt, ContractOptions as L, InferenceState as Lt, DefaultPackOptions as M, AgentMessage as Mt, defaultPack as N, CustomInferenceOptions as Nt, modelRequest as O, ValueOf as Ot, InferenceOptions as P, Infer as Pt, escalatableOf as Q, contract as R, ModelRequest as Rt, servedLog as S, resolve as St, turnOf as T, ModuleContext as Tt, morphCompaction as U, Usage as Ut, MorphOptions as V, NativeToolContext as Vt, naiveSummary as W, customInference as Wt, budgetSpent as X, budgetPhase as Y, canRequestBudget as Z, checkpointOf as _, ModuleManifest as _t, nativeTools as a, InboundMessage as at, suffixOf as b, canonicalValue as bt, CloudflareGatewayInferenceOptions as c, TurnOutcome as ct, EXITS as d, defineModule as dt, Agent as et, REQUEST_BUDGET as f, undeclaredEvents as ft, Checkpoint as g, Instruction as gt, keyOf as h, AgentProgram as ht, agentNativeTool as i, BranchOptions as it, systemPrompt as j, Action as jt, nativeToolSurface as k, provide as kt, cloudflareGatewayInference as l, TurnResult as lt, repairText as m, boundaryOf as mt, nudge as n, AgentServices as nt, VercelGatewayInferenceOptions as o, Module as ot, answerErrors as p, CallResult as pt, budget as q, AgentNativeToolOptions as r, AnyModule as rt, vercelGatewayInference as s, ModulePart as st, NudgeOptions as t, AgentOptions as tt, ANSWER as u, createAgent as ut, estimateTokens as v, Nudge as vt, turnHead as w, Binding as wt, replyView as x, programId as xt, keepUpTo as y, WITHDRAW_ALL as yt, COMPRESSION_RATIO as z, NativeTool as zt };